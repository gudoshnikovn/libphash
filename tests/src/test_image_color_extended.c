...
void test_hsv_classification_edges(void) {
    ...
    if (res.category != PH_HSV_BLACK) exit(1);

    // White
    res = ph_hsv_classify_pixel(255, 255, 255);
    if (res.category != PH_HSV_GRAY) exit(1); // S=0, so it's Gray

    // Gray
    res = ph_hsv_classify_pixel(128, 128, 128);
    if (res.category != PH_HSV_GRAY) exit(1);
...
void test_color_moments_edges(void) {
    ...
    if (m.mean < 84.0 || m.mean > 86.0) exit(1);
...
void test_pack_3bit_clamping(void) {
    ...
    if (!(hash & (1ULL << 41))) exit(1);
    if (!(hash & (1ULL << 40))) exit(1);
    if (!(hash & (1ULL << 39))) exit(1);
    if (hash & (1ULL << 38)) exit(1);
