...
void test_color_1ch_passthrough(void) {
    ...
    for(int i=0; i<4; i++) ASSERT_UINT8_EQ(src[i], dst[i]);
    
    ctx->image.width = 2; ctx->image.height = 2; ctx->image.channels = 1;
    ctx->image.raw_rgb = src; ctx->image.is_loaded = 1;
...
void test_get_gray_alloc(void) {
    ...
    uint8_t rgb[12] = {0};
    ctx->image.width = 2; ctx->image.height = 2; ctx->image.channels = 3;
    ...
    if (gray != ctx->image.gray_cache) exit(1);
...
void test_loader_exhaustion(void) {
    ...
    if (ph_decode_buffer(junk, 16, &w, &h, &ch, 0) != NULL) exit(1);
    if (ph_decode_buffer(NULL, 10, &w, &h, &ch, 0) != NULL) exit(1);
    if (ph_decode_buffer(junk, 0, &w, &h, &ch, 0) != NULL) exit(1);
