...
void benchmark_hash(const char *label, ph_error_t (*func)(ph_context_t *, uint64_t *),
                    ph_context_t *ctx, int iterations) {
    uint64_t hash;
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < iterations / 10; i++) {
        if (ph_compute_radial_hash(ctx, &digest) != PH_SUCCESS) {
            fprintf(stderr, "Radial hash failed\n");
        }
    }
    double end_rad = get_time_sec();
    printf("[BENCH] Radial     | %d iterations | Total: %.4fs | Avg: %.6fs/op\n", iterations / 10,
           end_rad - start_rad, (end_rad - start_rad) / (iterations / 10));
